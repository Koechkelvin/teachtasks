const calculateDashboard = (users) => {
    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);

    const activeUsers = users.filter(user => user.posts.some(post => new Date(post.timestamp) > oneWeekAgo));

    const popularPosts = activeUsers .map(user => user.posts.filter(post => new Date(post.timestamp) > oneWeekAgo && post.likes >= 10)).flat();
...
console.log(calculateDashboard(users));
/* output 
numberOfActiveUsers: 5,
totalNumberOfPOpularPosts: 3,
averageLikesPerUser: 8.2 */
